...
import { registerWithInviteAction } from "@/app/actions/auth";
...
export function InviteSignupForm({ email, token }: InviteSignupFormProps) {
  ...
  return (
    <div className="bg-card py-8 px-6 sm:px-10 rounded-3xl border border-border/50 shadow-xl space-y-6 text-left">
      {serverError && (
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="relative opacity-60">
            ...
            <Input
              ...
              disabled
              readOnly
              className="pl-10 h-11 rounded-xl bg-slate-50 border-border/50 text-[14px] cursor-not-allowed"
            />
          </div>
          <p className="text-[11.5px] text-foreground/40">This email is locked to your invitation.</p>
        </div>
        ...
        >
          {loading ? (
            <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Joining Workspace…</>
          ) : (
